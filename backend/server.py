...
async def create_visit(visit: VisitCreate, current_user: dict = Depends(get_current_user)):
    ...
    if isinstance(visit.visit_date, str):
        ...
        date_str = visit.visit_date.replace('Z', '').replace('T', ' ')
        ...
        try:
            ...
            visit_date_parsed = datetime.strptime(date_str, '%Y-%m-%d %H:%M:%S')
    else:
...
async def create_sale(sale: SaleCreate, current_user: dict = Depends(get_current_user)):
    ...
    if isinstance(sale.sale_date, str):
        # Handle different date formats
        date_str = sale.sale_date.replace('Z', '').replace('T', ' ')
        if '+' in date_str:
            date_str = date_str.split('+')[0]
        if '.' in date_str:
            date_str = date_str.split('.')[0]
        try:
            ...
            sale_date_parsed = datetime.strptime(date_str, '%Y-%m-%d %H:%M:%S')
    else:
...
async def create_collection(collection: CollectionCreate, current_user: dict = Depends(get_current_user)):
    ...
    if isinstance(collection.collection_date, str):
        # Handle different date formats
        date_str = collection.collection_date.replace('Z', '').replace('T', ' ')
        if '+' in date_str:
            date_str = date_str.split('+')[0]
        if '.' in date_str:
            date_str = date_str.split('.')[0]
        try:
            ...
            collection_date_parsed = datetime.strptime(date_str, '%Y-%m-%d %H:%M:%S')
    else:
